...
class ActivityPubPM:
    ...
    def pm_to_first_admin(self, message: str, domain: str, software: str):
        # TODO: Figure out how to get admin list from mastodon
        if software != "lemmy":
            return None
        lemmy = Lemmy(f"https://{domain}")
        site = lemmy.site.get()
        if not site:
            logger.warning(f"Could not get site info from lemmy domain {domain}")
            return
        admin_username = site["admins"][0]["person"]["name"]
        if not self.send_pm_to_right_software(
            message=message,
